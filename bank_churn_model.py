...
def select_csv(csvs):
    ...
    if ranked[0][0] < 8:
        raise ValueError(
            f"CSV found but it does not look like the expected churn dataset: {best}"
        )
...
def make_features(df):
    ...
    data["BalanceSalaryRatio"] = data["BalanceSalaryRatio"].replace(
        [np.inf, -np.inf], np.nan
    )

    data["AgeTenureRatio"] = data["Age"] / data["Tenure"].replace(0, np.nan)
    data["AgeTenureRatio"] = data["AgeTenureRatio"].replace(
        [np.inf, -np.inf], np.nan
    )

    data["BalancePerProduct"] = data["Balance"] / data["NumOfProducts"].replace(
        0, np.nan
    )
    data["BalancePerProduct"] = data["BalancePerProduct"].replace(
        [np.inf, -np.inf], np.nan
    )
    ...
    drop_cols = [
        c for c in ["RowNumber", "CustomerId", "Surname"] if c in data.columns
    ]
...
def add_features_for_prediction(df):
    ...
    df["BalanceSalaryRatio"] = df["Balance"] / df["EstimatedSalary"].replace(
        0, np.nan
    )
    df["BalanceSalaryRatio"] = df["BalanceSalaryRatio"].replace(
        [np.inf, -np.inf], np.nan
    )

    df["AgeTenureRatio"] = df["Age"] / df["Tenure"].replace(0, np.nan)
    df["AgeTenureRatio"] = df["AgeTenureRatio"].replace(
        [np.inf, -np.inf], np.nan
    )

    df["BalancePerProduct"] = df["Balance"] / df["NumOfProducts"].replace(
        0, np.nan
    )
    df["BalancePerProduct"] = df["BalancePerProduct"].replace(
        [np.inf, -np.inf], np.nan
    )
...
def train():
    ...
    X, y, _, drop_cols = make_features(df)

    # Hold the test split out completely until model selection and threshold tuning
    # are finished. This prevents optimistic test-set leakage.
    ...
    print(f"[2/4] Comparing {len(models)} candidate models on validation data...")
    for index, (name, model) in enumerate(models.items(), start=1):
        print(f"      {index}/{len(models)}  {name}")
        pipe = Pipeline(
            [("preprocess", clone(preprocessor)), ("model", model)]
        )
    ...
    leaderboard = (
        pd.DataFrame(rows)
        .sort_values(
            ["roc_auc", "f1", "average_dice", "accuracy"], ascending=False
        )
    ...
    final_metrics = evaluate(y_test, final_probs, best["threshold"])
    ...
    test_output["Actual_Status"] = np.where(
        test_output["Actual_Exited"] == 1,
        "LEFT_BANK_CHURNED",
        "CONTINUED_BANK",
    )
    ...
    test_output["Predicted_Status"] = np.where(
        test_output["Predicted_Exited"] == 1,
        "LEFT_BANK_CHURNED",
        "CONTINUED_BANK",
    )
    test_output.to_csv("outputs/test_predictions.csv", index=False)

    print("[4/4] Training complete. Review the held-out test summary below.")
    print("=" * 80)
    ...
    print("Dice continued:", round(final_metrics["dice_continued"], 4))
    print("Dice churned:", round(final_metrics["dice_churned"], 4))
    print("Average dice:", round(final_metrics["average_dice"], 4))
    print("\nConfusion matrix:")
    print(
        pd.DataFrame(
            final_cm,
            index=["Actual Continued", "Actual Churned"],
            columns=["Predicted Continued", "Predicted Churned"],
        )
    )
    ...
    print(
        classification_report(
            y_test,
            final_preds,
            target_names=["CONTINUED_BANK", "LEFT_BANK_CHURNED"],
            zero_division=0,
        )
    )
    print("\nArtifacts:")
...
def predict_customer(customer, model_path="outputs/bank_churn_model.joblib"):
    ...
    output["Predicted_Status"] = np.where(
        preds == 1, "LEFT_BANK_CHURNED", "CONTINUED_BANK"
    )
...
if __name__ == "__main__":
    ...
    print(
        sample_prediction[
            ["Churn_Probability", "Continue_Probability", "Predicted_Status"]
